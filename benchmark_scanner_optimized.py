...
class OptimizedPerformanceBenchmark:
    ...
    async def benchmark_batch_processing(self):
        ...
        for batch_size in batch_sizes:
            ...
            # 优化的处理函数 - 减少延迟，模拟真实的批处理
            async def process_batch(items):
                # 批处理通常比单个处理更高效
                # 模拟批处理优势：处理时间不是线性增长
                processing_time = 0.001 * (1 + len(items) * 0.01)  # 批处理效率提升
            ...
            # 使用优化的批处理流程
            results = await processor.process_all(items, process_batch)
            
            elapsed = time.time() - start
            rate = len(items) / elapsed if elapsed > 0 else 0
            
            print(f"  批大小 {batch_size:3d}: {rate:.1f} 项目/秒")
